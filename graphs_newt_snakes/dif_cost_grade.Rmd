...
```{r, graphing by gen}
trends_by_gen(cost_grade_lit, title="Space Tall: Newt VS Snake Population Size GA1", subt="Gen 16sims", y_label="Mean Phenotype", x_label="Generations", 100, 50000, rep=1)
...
whale_gen(cost_grade_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Phenotype", min_gen=min_gen, max_gen=max_gen, rep=rep )
```
...
```{r, red in grid files}
require(readr)
cost_grade_grid<- read.csv("~/Desktop/GA1_dif_cost/all_grid.txt" , header = TRUE) %>%  filter(between(generation, 9000, 10000))
