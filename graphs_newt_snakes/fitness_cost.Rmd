...
files <- list.files(path="~/Desktop/data", pattern="costtest", full.names=TRUE, recursive=FALSE)
number=1
file <- read.table(files[number], header = TRUE)
#file_name <- substr(files[number], 36, 186)
#print(paste0("File Name: ", file_name))

costnum <- as.numeric(unlist(strsplit(files[number], split= "_"))[30])
file$cost <-costnum


for(i in seq(2, length(files), 2)){
  temp_file <- read.table(files[i], header = TRUE)
  costnum<-as.numeric(unlist(strsplit(files[i], split= "_"))[30])
