...
SO cost = (phenotype/4.2)^2

I used 10 instead of 4.2 (due to math errors).
...
files <- list.files(path="~/Desktop/data/cost_test", pattern="costtest_", full.names=TRUE, recursive=FALSE)
number=1
file <- read.table(files[number], header = TRUE)
#file_name <- substr(files[number], 36, 186)
#print(paste0("File Name: ", file_name))

costnum <- as.numeric(unlist(strsplit(files[number], split= "_"))[31])
file$cost <-costnum


for(i in seq(2, length(files), 2)){
  temp_file <- read.table(files[i], header = TRUE)
  costnum<-as.numeric(unlist(strsplit(files[i], split= "_"))[31])
  temp_file$cost <-costnum
  file <- rbind(file, temp_file)
  
}
```

```{r, phenotypes at differnt costs figure}
pl1 <- ggplot(file, aes(gen, Newt_mean_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Newt Mean Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
  xlim(0, 5001)+
    theme_bw() 

pl2 <- ggplot(file, aes(gen, Snake_mean_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Mean Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 

pl3 <- ggplot(file, aes(gen, Newt_sd_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Newt SD Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 

pl4 <- ggplot(file, aes(gen, Snake_sd_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Sd Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 

pl5 <- ggplot(file, aes(gen, Newt_max_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Newt Max Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 

pl6 <- ggplot(file, aes(gen, Snake_max_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Max Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 

pl7 <- ggplot(file, aes(gen, Newt_min_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Newt Min Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 

pl8 <- ggplot(file, aes(gen, Snake_min_Pheno, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Min Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 


grid.arrange(pl1, pl2, nrow=1)
grid.arrange(pl3, pl4, nrow=1)
grid.arrange(pl5, pl6, nrow=1)
grid.arrange(pl7, pl8, nrow=1)
...
```{r, cost and the dif between snake and newt}
file$dif <- file$Snake_mean_Pheno-file$Newt_mean_Pheno

ggplot(file, aes(gen, dif, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Mean Pheno by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype")+
    theme_bw() 
...
Is there a point where the cost of a phenotype reaches a balance with selection pressure experienced by the population?

```{r, cost equlibrium}

file$newt_equl <- file$Newt_mean_Pheno/file$cost
file$snake_equl <- file$Snake_mean_Pheno/file$cost

e1 <- ggplot(file, aes(gen, newt_equl, col=factor(cost))) + geom_line()+ 
    ggtitle("Newt Mean Pheno/Cost by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype/Cost")+
    theme_bw() 

e2 <- ggplot(file, aes(gen, snake_equl, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Mean Pheno/Cost by Gen", subtitle = "Different Cost") +
    xlab("Generations") + ylab("Phenotype/Cost")+
    theme_bw() 
...
```{r, population size equlibrium}

pop1 <- ggplot(file, aes(gen, Newt_pop_size, col=factor(cost))) + geom_line()+ 
    ggtitle("Newt Popsize by Gen", subtitle = "Different Cost") +
    xlab("Popsize") + ylab("Phenotype/Cost")+
    theme_bw() 

pop2 <- ggplot(file, aes(gen, Snake_pop_size, col=factor(cost))) + geom_line()+ 
    ggtitle("Snake Popsize by Gen", subtitle = "Different Cost") +
    xlab("Popsize") + ylab("Phenotype/Cost")+
    theme_bw() 


grid.arrange(pop1, pop2, nrow=1)
```

```{r, is this pheno what I expect}
phenotype <- c(0:200)
plot(phenotype,exp(-(phenotype/200)^2), type = "l", main="Fitness Cost", xlab="Phenotype", ylab="Cost")
lines(phenotype,exp(-(phenotype/150)^2))
lines(phenotype,exp(-(phenotype/100)^2))
lines(phenotype,exp(-(phenotype/50)^2))
lines(phenotype,exp(-(phenotype/10)^2))
```
