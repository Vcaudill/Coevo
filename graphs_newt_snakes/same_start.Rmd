...
Start all simultions off with the same msprime paramiters to see if the final results change. My prediction is that they will not.
...
same_start_lit<- read.csv("~/Desktop/same_start/all_lit.txt" , header = TRUE)

same_start_lit$group_rep <- paste0(same_start_lit$snake_mu_rate, "_", same_start_lit$snake_mu_effect_sd,"_",same_start_lit$newt_mu_rate, "_", same_start_lit$newt_mu_effect_sd, "_", same_start_lit$rep)

same_start_lit$group <- paste0(same_start_lit$snake_mu_rate, "_", same_start_lit$snake_mu_effect_sd,"_",same_start_lit$newt_mu_rate, "_", same_start_lit$newt_mu_effect_sd)

same_start_lit$sim=LETTERS[factor(same_start_lit$group)]
same_start_lit$dif <- same_start_lit$Snake_mean_Pheno-same_start_lit$Newt_mean_Pheno


same_start_cor<- read.csv("~/Desktop/same_start/all_cor.txt" , header = TRUE)
same_start_cor$group_rep <- paste0(same_start_cor$snake_mu_rate, "_", same_start_cor$snake_mu_effect_sd,"_",same_start_cor$newt_mu_rate, "_", same_start_cor$newt_mu_effect_sd, "_", same_start_cor$rep)
same_start_cor$group <- paste0(same_start_cor$snake_mu_rate, "_", same_start_cor$snake_mu_effect_sd,"_",same_start_cor$newt_mu_rate, "_", same_start_cor$newt_mu_effect_sd)
same_start_cor$sim=LETTERS[factor(same_start_cor$group)]
...
#two_hist_death_all_rep(same_start_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype (purple) & Snake-Newt Popsize/100 (green)",  min_gen=20000, max_gen=25000)
...
```{r, graphing by gen}
trends_by_gen(same_start_lit, title="Space Tall: Newt VS Snake Population Size GA1", subt="Gen 16sims", y_label="Mean Phenotype", x_label="Generations", 100, 50000, rep=1)


#aggregate(same_start_lit[which(same_start_lit$generation>=100 & same_start_lit$generation<=50000),]$dif, list(same_start_lit[which(same_start_lit$generation>=100 & same_start_lit$generation<=50000),]$group_rep), FUN=mean) 
...
aggregate(same_start_lit[which(same_start_lit$generation>=100 & same_start_lit$generation<=50000),]$dif, list(same_start_lit[which(same_start_lit$generation>=100 & same_start_lit$generation<=50000),]$group), FUN=mean) 
```
...
whale_gen(same_start_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Phenotype", min_gen=min_gen, max_gen=max_gen, rep=rep )
```

### Phenotype & Populationsize differences
```{r, popsize and pheno}

suppressMessages(print(two_hist(same_start_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Popsize/1000 (green) & Snake-Newt Phenotype (purple)",  min_gen=min_gen, max_gen=max_gen, rep=rep )))

#suppressMessages(print(two_hist_all_rep(same_start_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Popsize/100 (green) & Snake-Newt Phenotype (purple)",  min_gen=min_gen, max_gen=max_gen)))
...
boxplot_cors_by_gen(same_start_cor, "Simulation Correlation Comparied to Real Correlation Trials", "GA1", "newt/snake GAs&rep", "Cor", ginmin, ginmax)


#four_types_boxplot_cors_by_gen(same_start_cor, "Simulation four Correlation", "GA1", "newt/snake GAs&rep", "Cor", ginmin, ginmax)
...
ginmin = 0
ginmax = 5000
suppressMessages(print(hist_cors_by_gen_col_rep(same_start_cor, "Simulation Correlation Comparied to Real Correlation", "GA1", "newt/snake GAs&rep", "Cor", ginmin, ginmax)))
```
...
### Random 1  
```{r, GA1 cor and pheno}
GA1_pattern <- unique(paste0(same_start_lit$snake_mu_rate, "_", same_start_lit$snake_mu_effect_sd,"_",same_start_lit$newt_mu_rate, "_", same_start_lit$newt_mu_effect_sd, "_", same_start_lit$rep))

myrandomlist=sample.int(length(GA1_pattern), 3)

# for(i in myrandomlist){
#   myine<-cor_trends_by_gen(same_start_lit, same_start_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[i])
#   plot(myine)
# }

cor_trends_by_gen(same_start_lit, same_start_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[myrandomlist[1]])
...
cor_trends_by_gen(same_start_lit, same_start_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[myrandomlist[2]])
```

### Random 3  
```{r, random cor-pheno plot 3}

cor_trends_by_gen(same_start_lit, same_start_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[myrandomlist[3]])
```
...
# for(i in 1:length(unique(same_start_lit$rep))){
#   myine<-whale_gen(same_start_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize", min_gen=min_gen, max_gen=max_gen, rep=unique(same_start_lit$rep)[i])
#   plot(myine)
# 
# }
whale_gen(same_start_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize", min_gen=min_gen, max_gen=max_gen, rep=unique(same_start_lit$rep)[1])
```
...
# for(i in 1:length(unique(same_start_lit$rep))){
#   myine<-two_hist(same_start_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype (purple) & Snake-Newt Popsize/100 (green)",  min_gen=min_gen, max_gen=max_gen, rep=unique(same_start_lit$rep)[i] )
#   suppressMessages(plot(myine))
# }

two_hist(same_start_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype (purple) & Snake-Newt Popsize/100 (green)",  min_gen=min_gen, max_gen=max_gen, rep=unique(same_start_lit$rep)[1] )
...
min_gen=100
max_gen=5000
GA_summary(same_start_lit, min_gen=min_gen, max_gen=max_gen, x_line="Newt_pop_size", y_line="Snake_pop_size", title="Space: Newt VS Snake Population Size", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize")
```
...
GA_summary(same_start_lit, min_gen=min_gen, max_gen=max_gen, x_line="dif", y_line="Snake_pop_size", title="Space: Dif VS Snake Population Size", subt="Gen 16sims", y_label="Snake Popsize", x_label="Dif")
```
...
GA_summary_by_species(same_start_lit, min_gen=min_gen, max_gen=max_gen, x_line="dif", y_line="snake", title="Space: Dif VS Snake GA", subt="Gen 16sims", y_label="Snake GA", x_label="Dif")
...
GA_summary_by_species(same_start_lit, min_gen=min_gen, max_gen=max_gen, x_line="dif", y_line="newt", title="Space: Dif VS Newt GA", subt="Gen 16sims", y_label="Newt GA", x_label="Dif")
...
max_gen=5000
#heatmap
build_heatmap(same_start_lit, min_gen=min_gen, max_gen=max_gen, fill_line="Snake_pop_size", fill_line_text="Snake Population Size", title="Newt GA by Snake GA", subt="16 Sim Heatmap", y_label="Snake GA", x_label="Newt GA", red_blue=F)
```
...
min_gen=100
max_gen=5000
build_heatmap(same_start_lit, min_gen=min_gen, max_gen=max_gen, fill_line="dif", fill_line_text="Snake-Newt", title="Newt GA by Snake GA", subt="16 Sim Heatmap", y_label="Snake GA", x_label="Newt GA",red_blue=T)
```
...
```{r, red in grid files}
require(readr)
same_start_grid<- read.csv("~/Desktop/same_start/all_grid.txt" , header = TRUE) %>%  filter(between(generation, 9000, 10000))
same_start_grid$group_rep <- paste0(same_start_grid$snake_mu_rate, "_", same_start_grid$snake_mu_effect_sd,"_",same_start_grid$newt_mu_rate, "_", same_start_grid$newt_mu_effect_sd, "_", same_start_grid$rep)
same_start_grid$group <- paste0(same_start_grid$snake_mu_rate, "_", same_start_grid$snake_mu_effect_sd,"_",same_start_grid$newt_mu_rate, "_", same_start_grid$newt_mu_effect_sd)
same_start_grid$sim=LETTERS[factor(same_start_grid$group)]
same_start_grid$grid_num = 5
...
require(readr)
same_start_grid<- read.csv("~/Desktop/same_start/all_grid.txt" , header = TRUE) %>%  filter(between(generation, 40000, 50000))
same_start_grid$group_rep <- paste0(same_start_grid$snake_mu_rate, "_", same_start_grid$snake_mu_effect_sd,"_",same_start_grid$newt_mu_rate, "_", same_start_grid$newt_mu_effect_sd, "_", same_start_grid$rep)
same_start_grid$group <- paste0(same_start_grid$snake_mu_rate, "_", same_start_grid$snake_mu_effect_sd,"_",same_start_grid$newt_mu_rate, "_", same_start_grid$newt_mu_effect_sd)
same_start_grid$sim=LETTERS[factor(same_start_grid$group)]
same_start_grid$grid_num = 5
