...
dif
# for the poster
# title="Newt & Snake Mean Phenotype as Generations Increases"
# subt="Newt & Snake GA: Mutation Rate 1e-10, Effect size 0.5"
# y_label="Mean Phenotype"
# x_label="Generations"
# ggplot(GA_file_100[ which(GA_file_100$gen!=1 & GA_file_100$sim=="K"),]) +
#   xlim(0, 30000)+
#   ggtitle(title, subtitle = subt) +
#   xlab(x_label) + ylab(y_label)+
#   geom_line(aes(x=as.numeric(gen), y=as.numeric(Snake_mean_Pheno)),size=1, color = "steelblue")+
#   geom_line(aes(x=as.numeric(gen), y=as.numeric(Newt_mean_Pheno)),size=1, color = "darkred")+
#   theme(plot.title = element_text(size=5)) +
#   #scale_colour_gradient2(high = "steelblue", mid = "beige", low = "darkred")+
#   #labs(colour = color_label) +
#   #facet_wrap(~sim)+
