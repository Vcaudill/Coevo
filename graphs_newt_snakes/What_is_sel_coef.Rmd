...
p1<-ggplot(file, aes(phenotype)) + 
  geom_histogram(col = "darkred", fill = "darkred", binwidth=30)+ 
  theme_bw()+
  scale_y_log10()+
  xlim(-1, 1000) +
...
labs(title = 'Newt Phenotype Dist. Generation: {frame_time}', x = 'Phenotype', y = 'Count') +
  transition_time(gen) +
  ease_aes('linear')
animate(p1, duration = 25, fps = 30, width = 400, height = 400, renderer = gifski_renderer())
...
labs(title = 'Generation: {frame_time}', x = 'Phenotype', y = 'Fitness') +
  transition_time(gen) +
  ease_aes('linear')
animate(p, duration = 25, fps = 30, width = 400, height = 400, renderer = gifski_renderer())
