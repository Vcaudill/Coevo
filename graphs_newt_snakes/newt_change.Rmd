...
To test what would happen with the spatial phenotype correlation between newts and snakes when the fitness cost for newts in on a scale (dependent on location) and the fitness cost for snakes is constant. 
...
Each GA combination and trial has its own msprime simulation. I created a gradient map where at the top there is a higher fitness cost for having a large phenotype and at the bottom there is a lower cost for having a higher phenotype. Only newts are effected by this gradient. Snakes fitness cost remain at 100. To see the gradient figure see cost gradient file.
...
newt_change_lit<- read.csv("~/Desktop/newt_change/all_lit.txt" , header = TRUE)

newt_change_lit$group_rep <- paste0(newt_change_lit$snake_mu_rate, "_", newt_change_lit$snake_mu_effect_sd,"_",newt_change_lit$newt_mu_rate, "_", newt_change_lit$newt_mu_effect_sd, "_", newt_change_lit$rep)

newt_change_lit$group <- paste0(newt_change_lit$snake_mu_rate, "_", newt_change_lit$snake_mu_effect_sd,"_",newt_change_lit$newt_mu_rate, "_", newt_change_lit$newt_mu_effect_sd)

newt_change_lit$sim=LETTERS[factor(newt_change_lit$group)]
newt_change_lit$dif <- newt_change_lit$Snake_mean_Pheno-newt_change_lit$Newt_mean_Pheno
...
#file=newt_change_lit
#cfile=newt_change_cor 
...
#two_hist_death_all_rep(newt_change_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype (purple) & Snake-Newt Popsize/100 (green)",  min_gen=20000, max_gen=25000)
...
```{r, graphing by gen}
trends_by_gen(newt_change_lit, title="Space Tall: Newt VS Snake Population Size GA1", subt="Gen 16sims", y_label="Mean Phenotype", x_label="Generations", 100, 50000, rep=1)


#aggregate(newt_change_lit[which(newt_change_lit$generation>=100 & newt_change_lit$generation<=50000),]$dif, list(newt_change_lit[which(newt_change_lit$generation>=100 & newt_change_lit$generation<=50000),]$group_rep), FUN=mean) 
...
aggregate(newt_change_lit[which(newt_change_lit$generation>=100 & newt_change_lit$generation<=50000),]$dif, list(newt_change_lit[which(newt_change_lit$generation>=100 & newt_change_lit$generation<=50000),]$group), FUN=mean) 
```


## {-}

These results look very similar to the results seen in my other simulations (no gradient map and gradient map). The mean phenotypes of newts and snakes go up, until they reach an equilibrium. Here the average values are higher than what I have seen in the no gradent map, but are lower than what I see in the cost gradient map.
...
whale_gen(newt_change_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize", min_gen=min_gen, max_gen=max_gen, rep=rep )
```

### Phenotype & Populationsize differences
```{r, popsize and pheno}

suppressMessages(print(two_hist(newt_change_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Popsize/1000 (green) & Snake-Newt Phenotype (purple)",  min_gen=min_gen, max_gen=max_gen, rep=rep )))

#suppressMessages(print(two_hist_all_rep(newt_change_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Popsize/100 (green) & Snake-Newt Phenotype (purple)",  min_gen=min_gen, max_gen=max_gen)))
...
boxplot_cors_by_gen(newt_change_cor, "Simulation Correlation Comparied to Real Correlation Trials", "GA1", "newt/snake GAs&rep", "Cor", ginmin, ginmax)


#four_types_boxplot_cors_by_gen(newt_change_cor, "Simulation four Correlation", "GA1", "newt/snake GAs&rep", "Cor", ginmin, ginmax)


```

In my previous results (no gradient map) I saw no positive spatial phenotype correlation between snakes and newts. Here, with a cost gradient map for newts I see that there are quite a few simulations where newt and snake local mean phenotypes are correlated when compared across space. This correlation is not as high or as frequent as what I saw when both newt and snake fitness cost was dependent on location.
...
ginmin = 0
ginmax = 5000
suppressMessages(print(hist_cors_by_gen_col_rep(newt_change_cor, "Simulation Correlation Comparied to Real Correlation", "GA1", "newt/snake GAs&rep", "Cor", ginmin, ginmax)))
```
...
Spatial phenotype correlation increases for most of the simulations as time increases. Once a simulation reach a high spatial phenotype correlation, it typically did not go back down. The simulations where the simulations correlation values did not come close to the empirical newt and snake spatial phenotype correlation all contained a genetic architecture that had a small mutational variance. The exception to this is in the simulation where both newts and snakes had a low mutational variance, the local phenotypes became correlated then it faded (I wonder why). The spatial phenotype correlation is a lot higher than when there was not gradient map, but lower than when both newt and snakes fitness cost was on a gradient. When compared to the correlation results of snakes on a cost gradient and newts constant, the results in this document are more spatially correlated. 
...
### Random 1  
```{r, GA1 cor and pheno}
GA1_pattern <- unique(paste0(newt_change_lit$snake_mu_rate, "_", newt_change_lit$snake_mu_effect_sd,"_",newt_change_lit$newt_mu_rate, "_", newt_change_lit$newt_mu_effect_sd, "_", newt_change_lit$rep))

myrandomlist=sample.int(length(GA1_pattern), 3)

# for(i in myrandomlist){
#   myine<-cor_trends_by_gen(newt_change_lit, newt_change_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[i])
#   plot(myine)
# }

cor_trends_by_gen(newt_change_lit, newt_change_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[myrandomlist[1]])
...
cor_trends_by_gen(newt_change_lit, newt_change_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[myrandomlist[2]])
```

### Random 3  
```{r, random cor-pheno plot 3}

cor_trends_by_gen(newt_change_lit, newt_change_cor, "test", "GA1", "newt/snake GAs&rep","other", "Cor", min_gen=100, max_gen=50000, pattern=GA1_pattern[myrandomlist[3]])
```
...
# for(i in 1:length(unique(newt_change_lit$rep))){
#   myine<-whale_gen(newt_change_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize", min_gen=min_gen, max_gen=max_gen, rep=unique(newt_change_lit$rep)[i])
#   plot(myine)
# 
# }
whale_gen(newt_change_lit, title="Space: Newt VS Snake Population Size Color=S-N Phenotype", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize", min_gen=min_gen, max_gen=max_gen, rep=unique(newt_change_lit$rep)[1])
```
...
# for(i in 1:length(unique(newt_change_lit$rep))){
#   myine<-two_hist(newt_change_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype (purple) & Snake-Newt Popsize/100 (green)",  min_gen=min_gen, max_gen=max_gen, rep=unique(newt_change_lit$rep)[i] )
#   suppressMessages(plot(myine))
# }

two_hist(newt_change_lit, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype (purple) & Snake-Newt Popsize/100 (green)",  min_gen=min_gen, max_gen=max_gen, rep=unique(newt_change_lit$rep)[1] )
...
The results looked similar to results that I have seen in other experiments. In the beginning of the simulation both newt and snake population grows. The difference in phenotype quickly becomes polarized. The population size reaches a steady point and then newts and snakes co-evolve. In the middle part of my simulation, the difference between newt and snakes mean phenotype solidifies. Often, the difference in mean phenotype decreases (where compared to the beginning of the simulation). When newts had a GA that with a high mutation rate and low mutation effect size (GA 1), the difference in mean phenotype grows, leading to the newts losing the co-evolutionary arms race. In all other GA combinations newts had a larger phenotype. When looking at the snakes on a cost fitness scale, the results are flipped. When snakes had a GA that with a high mutation rate and low mutation effect size (GA 1), leading to the snakes losing the co-evolutionary arms race. Could there be something with in the environments causing one species to out compete the other? The histograms reflect what is seen in the scatter plots.
...
min_gen=100
max_gen=5000
GA_summary(newt_change_lit, min_gen=min_gen, max_gen=max_gen, x_line="Newt_pop_size", y_line="Snake_pop_size", title="Space: Newt VS Snake Population Size", subt="Gen 16sims", y_label="Snake Popsize", x_label="Newt Popsize")
```
...
GA_summary(newt_change_lit, min_gen=min_gen, max_gen=max_gen, x_line="dif", y_line="Snake_pop_size", title="Space: Dif VS Snake Population Size", subt="Gen 16sims", y_label="Snake Popsize", x_label="Dif")
```
...
GA_summary_by_species(newt_change_lit, min_gen=min_gen, max_gen=max_gen, x_line="dif", y_line="snake", title="Space: Dif VS Snake GA", subt="Gen 16sims", y_label="Snake GA", x_label="Dif")
...
GA_summary_by_species(newt_change_lit, min_gen=min_gen, max_gen=max_gen, x_line="dif", y_line="newt", title="Space: Dif VS Newt GA", subt="Gen 16sims", y_label="Newt GA", x_label="Dif")
...
Firstly, these plots are very similar to the plots seen with and without a gradient map. There are clusters of points that create lines of shapes and lines of colors. Points began clustered then spread apart. These points are similar between trials. There is one striking differences in these results, it seems that the results are shifted toward the newts favor (in the snakes cost gradient results things are shifted towards the snakes side). The shift does not seem to effect the circles (newt GA1). Thes plots are fun to look at.
...
max_gen=5000
#heatmap
build_heatmap(newt_change_lit, min_gen=min_gen, max_gen=max_gen, fill_line="Snake_pop_size", fill_line_text="Snake Population Size", title="Newt GA by Snake GA", subt="16 Sim Heatmap", y_label="Snake GA", x_label="Newt GA", red_blue=F)
```
...
min_gen=100
max_gen=5000
build_heatmap(newt_change_lit, min_gen=min_gen, max_gen=max_gen, fill_line="dif", fill_line_text="Snake-Newt", title="Newt GA by Snake GA", subt="16 Sim Heatmap", y_label="Snake GA", x_label="Newt GA",red_blue=T)
```
...
Heatmap results are different, the seem more muted (less differences between GA combinations). They start off sort of neutral, and then become set in the favor of the newt (snake has a smaller population size while newts have a larger phenotype). Each trial of the GA combination creates very similar patterns.


```{r, red in grid files}
require(readr)
newt_change_grid<- read.csv("~/Desktop/newt_change/all_grid.txt" , header = TRUE) %>%  filter(between(generation, 9000, 10000))
newt_change_grid$group_rep <- paste0(newt_change_grid$snake_mu_rate, "_", newt_change_grid$snake_mu_effect_sd,"_",newt_change_grid$newt_mu_rate, "_", newt_change_grid$newt_mu_effect_sd, "_", newt_change_grid$rep)
newt_change_grid$group <- paste0(newt_change_grid$snake_mu_rate, "_", newt_change_grid$snake_mu_effect_sd,"_",newt_change_grid$newt_mu_rate, "_", newt_change_grid$newt_mu_effect_sd)
newt_change_grid$sim=LETTERS[factor(newt_change_grid$group)]
newt_change_grid$grid_num = 5
...
require(readr)
newt_change_grid<- read.csv("~/Desktop/newt_change/all_grid.txt" , header = TRUE) %>%  filter(between(generation, 40000, 50000))
newt_change_grid$group_rep <- paste0(newt_change_grid$snake_mu_rate, "_", newt_change_grid$snake_mu_effect_sd,"_",newt_change_grid$newt_mu_rate, "_", newt_change_grid$newt_mu_effect_sd, "_", newt_change_grid$rep)
newt_change_grid$group <- paste0(newt_change_grid$snake_mu_rate, "_", newt_change_grid$snake_mu_effect_sd,"_",newt_change_grid$newt_mu_rate, "_", newt_change_grid$newt_mu_effect_sd)
newt_change_grid$sim=LETTERS[factor(newt_change_grid$group)]
newt_change_grid$grid_num = 5
