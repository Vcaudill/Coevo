...
plot <-   ggplot(file_long) +
  ggtitle(title, subtitle = subt)+
  xlab(x_label) + ylab(y_label)+
geom_boxplot(aes(x=factor(sim), y=as.numeric(value), fill=factor(name), alpha = rect_type_a))+ #, level=c("D", "C", "B", "H", "G", "L", "A", "F", "K", "P", "E", "J", "I", "O", "N", "M")
...
file$coevo_speed[file$sim %in% choices[1]] = "Fast Coevolution"
file$coevo_speed[file$sim %in% choices[2]] = "Slow Coevolution"
file$coevo_speed[file$sim %in% choices[3]] = "No Coevolution"
file$coevo_speed = factor(file$coevo_speed, levels=c("Fast Coevolution", "Slow Coevolution", "No Coevolution"))
...
boxplot_coevo_slope_flat_map(file3, title, subt, x_label, y_label)
...
x_label= "Generations"
y_label= "Phenotype"
fast_slow_no_coevolution(file, choices, min_gen, max_gen, title, subt, x_label, y_label)
```
