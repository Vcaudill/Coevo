...
file1 = GA1_cor[which(1:nrow(GA1_cor) %% 2 != 0) , ]
...
file_long <- tidyr::pivot_longer(file, c(newt_pheno_change, snake_pheno_change))
  ...
  ggtitle(title, subtitle = subt)+
  xlab(x_label) + ylab(y_label)+
  geom_boxplot(aes(x=factor(sim, level=c("D", "C", "B", "H", "G", "L", "A", "F", "K", "P", "E", "J", "I", "O", "N", "M")), y=as.numeric(value), fill=factor(name)))+
  theme(plot.title = element_text(size=5)) +
  #xlim(-1.1, 1.1)+
  scale_fill_manual(values =c("darkred", "steelblue"), labels=c('Newt', 'Snake'))+
    labs(fill = " ")+
...
ggplot(GA_stage_long) +
  ...
  scale_fill_manual(values =c("darkred", "steelblue"), labels=c('Newt', 'Snake'))+ #
  #  labs(fill = " ")
  scale_x_discrete(labels=c('low/low', 'low/high', 'low/high','high/high'))+
  theme_bw() +  #theme(axis.text.x = element_text(angle = 45, hjust=1, size = 8), 
                 #     title =element_text(size=8, face='bold'))+
