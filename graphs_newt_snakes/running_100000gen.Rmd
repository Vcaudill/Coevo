...
trends_by_gen <- function(file, title, subt, x_label, y_label, min_gen, max_gen){
  ...
  plot <- ggplot(file[ which(file$generation>=min_gen & file$generation<=max_gen),]) +
    ...
    geom_line(aes(generation, Snake_mean_Pheno, group=rep, color="Snake_mean_Pheno"),)+
    geom_line(aes(generation, Newt_mean_Pheno, group=rep, color="Newt_mean_Pheno"))+
    geom_hline(yintercept=0, linetype="dashed", color="gray54", size=0.7)+
    ylim(-7,10)+
    xlab(x_label) + ylab(y_label)+
    scale_color_manual(name = "Type",
...
two_hist_all_rep(GA1_long_file, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype/100 (green) & Snake-Newt Popsize (purple)",  min_gen=25000, max_gen=30000)
...
```{r, graphing by gen}
trends_by_gen(GA1_long_file, title="Space: Newt VS Snake Population Size GA1", subt="Gen 16sims", y_label="Mean Phenotype", x_label="Generations", 100, 50000)
...
ggsave(
  "~/Desktop/newt_snake_coevo_16.pdf",
  plot = last_plot(),
  scale = 1,
  width = 7,
  height = 5,
...
```{r, the scatter of pheno and pop size}
min_gen=5000
max_gen=15000

rep=2

drop_lows <- GA1_long_file[which(GA1_long_file$generation>=min_gen & GA1_long_file$generation<=max_gen & GA1_long_file$rep==rep),]

drop_lows$popsize_dif<-drop_lows$Snake_pop_size-drop_lows$Newt_pop_size
mean(abs(drop_lows$dif))
value=sd(abs(drop_lows$dif))*3

#drop_lows[which(drop_lows$dif>0),]$Newt_mean_Pheno = NA
#drop_lows[which(drop_lows$dif>0),]$Newt_pop_size = NA
#drop_lows[which(drop_lows$dif<0),]$Snake_mean_Pheno = NA
#drop_lows[which(drop_lows$dif<0),]$Snake_pop_size = NA

h <- hist(drop_lows$dif, breaks=18, plot=FALSE)
cuts <- cut(h$breaks, c(-Inf,(abs(min(drop_lows$dif))-value)+min(drop_lows$dif),
                        max(drop_lows$dif)-(max(drop_lows$dif)-value+.5),max(drop_lows$dif)))


#pdf("~/Desktop/dif_in_phenotype_hist.pdf", width = 7, height = 5)
plot(h, col=c("yellow","grey","yellow")[cuts])
#dev.off()


# ggplot(drop_lows[which(abs(drop_lows$dif)>=value),]) +
#   #xlim(0, 1000)+
#   #ggtitle(title, subtitle = paste0(subt, " From ", min_gen, " gen to ", max_gen, " gen")) +
#   #xlab(x_label) + ylab(y_label)+
#   geom_point(aes(x=as.numeric(Newt_mean_Pheno), y=abs(popsize_dif)), color = "darkred",size=3)+
#   geom_point(aes(x=as.numeric(Snake_mean_Pheno), y=abs(popsize_dif)), color = "steelblue",size=3)+
# #  geom_point(aes(x=as.numeric(Newt_pop_size_20), y=as.numeric(Snake_pop_size_20), color = factor(snake), shape=factor(newt)))+
#   geom_smooth(aes(x=Newt_mean_Pheno, y=abs(popsize_dif)), method=lm, se=FALSE, fullrange=TRUE, color = "darkred")+
#   geom_smooth(aes(x=Snake_mean_Pheno, y=abs(popsize_dif)), method=lm, se=FALSE, fullrange=TRUE, color = "steelblue")+
#   theme(plot.title = element_text(size=5)) +
#   #scale_colour_gradient2(high = "steelblue", mid = "beige", low = "darkred")+
#   #labs(colour = color_label) +
#   #facet_wrap(~sim)+
#   theme_bw() +  theme(axis.text.x = element_text( hjust=1, size = 8))
# 
# ggsave("~/Desktop/dif_in_phenotype_popsize.pdf", plot = last_plot(), scale = 1,
#   width = 7, height = 5, dpi = 300, limitsize = TRUE, bg = NULL)

ggplot(drop_lows)  +
 ggtitle(paste0(subt, " From ", min_gen, " gen to ", max_gen, " gen")) +
  geom_point(aes(x=as.numeric(dif), y=popsize_dif), color = "darkgreen",size=3)+
  theme(plot.title = element_text(size=5)) +
  theme_bw() +  theme(axis.text.x = element_text( hjust=1, size = 8))
ggsave("~/Desktop/dif_phenotype_dif_popsize.pdf", plot = last_plot(), scale = 1,
  width = 7, height = 5, dpi = 300, limitsize = TRUE, bg = NULL)
...
ggplot(drop_lows[which(abs(drop_lows$dif)>=value),])  +
  ...
  #ggtitle(title, subtitle = paste0(subt, " From ", min_gen, " gen to ", max_gen, " gen")) +
  #xlab(x_label) + ylab(y_label)+
  geom_point(aes(x=as.numeric(Newt_mean_Pheno), y=abs(Newt_pop_size)), color = "darkred",size=3)+
  geom_point(aes(x=as.numeric(Snake_mean_Pheno), y=abs(Snake_pop_size)), color = "steelblue",size=3)+
#  geom_point(aes(x=as.numeric(Newt_pop_size_20), y=as.numeric(Snake_pop_size_20), color = factor(snake), shape=factor(newt)))+
  #geom_smooth(aes(x=Newt_mean_Pheno, y=Newt_pop_size), method=lm, se=FALSE, fullrange=TRUE, color = "darkred")+
  #geom_smooth(aes(x=Snake_mean_Pheno, y=Snake_pop_size), method=lm, se=FALSE, fullrange=TRUE, color = "steelblue")+
  theme(plot.title = element_text(size=5)) +
  #scale_colour_gradient2(high = "steelblue", mid = "beige", low = "darkred")+
  #labs(colour = color_label) +
  #facet_wrap(~sim)+
  theme_bw() +  theme(axis.text.x = element_text( hjust=1, size = 8))
#ggsave("~/Desktop/phenotype_popsize.pdf", plot = last_plot(), scale = 1,
 # width = 7, height = 5, dpi = 300, limitsize = TRUE, bg = NULL)
...
two_hist_all_rep(GA1_long_file, title="Snake-Newt Winning Hist", subt="Gen 16sims", y_label="Count", x_label="Snake-Newt Phenotype/100 (green) & Snake-Newt Popsize (purple)",  min_gen=min_gen, max_gen=max_gen)
