...
p <- ggplot(file1, aes(x=gen)) +
  ...
  ggtitle("Wrong1sim: Phenotype Measurements for Newts and Snakes") +
...
for (current_file in files){
  ...
  file <- file[complete.cases(file), ]
 p <- p + geom_point(data= file, x = i, aes(y = as.numeric(Newt_mean_Pheno[1])), color = darken("darkred")) +
   geom_point(data= file,x = i, aes(y = as.numeric(Newt_mean_Pheno[50])), color = lighten("darkred"))  + geom_point(data= file, x = i, aes(y = as.numeric(Snake_mean_Pheno[1])), color = darken("steelblue")) +
  geom_point(data= file, x = i, aes(y = as.numeric(Snake_mean_Pheno[50])), color = lighten("steelblue"))
 i = i+1
}
