...
## Fake Example of the Problem I have been seeing 

```{r fake exaple, echo=FALSE}
gen <- seq(20, 1000, by = 20)
  ex <- as.data.frame(gen)

ex$Newt_mean_Pheno <- pexp(seq(0.2, 10, by = .2), rate = 1)*seq(1, 100, by = 2) + rnorm(50, mean = 0, sd = 5)
  ex$Snake_mean_Pheno <-seq(.18, 5.1, by = .1) + + rnorm(50, mean = 0, sd = 5)
p <- ggplot(ex, aes(x=gen)) +
  xlim(0, 1000)+
  #ylim(0, true_max_1)+
  geom_line(aes(y = Newt_mean_Pheno, colour="Newt")) + 
  geom_line(aes(y = Snake_mean_Pheno, colour="Snake"), linetype="twodash")+
  ggtitle("Fake Example of What I was Observing") +
...
I used different values for snake_mu_effect_sd	newt_mu_effect_sd	snake_mu_rate	newt_mu_rate. I chose these parameters to see if there would be a see-able interaction in phenotypes 
Other params:
...
Then set up a graph with the average phenotype per generation:
I collected data ever 20 generations until 1000 generations were done
...
Here I have a questions why are the newts always starting off higher than the snakes????
...
I ran 144 SLiM simulations(might be 143?) with different parameters values and used the MSprime file: both_su_1e-10_nu_1e-10_sue_0.01_nue_0.01 (s/n u mutation rate, n/s ue mutation effect size) 
...
```
I noticed that in a majority of these the newts had a higher phenotype than the snakes.
...
**I found an error in the code which was not giving the correct mutation rate to snakes**
...
Some of these values seem very large, but the means are not super bad.
...
Some of these simulations still created very large phenotypes. I would like to know why
*Was it the mutation or effect size values?
*Is it the msprime simulation starting point?
*Is the maximum value important? And if so what does it mean?
*Compare simulated phenotype values to the predicted values from the equation mediated by the parameters
*Is this caused by space?
*Is it caused by co-evolution?
