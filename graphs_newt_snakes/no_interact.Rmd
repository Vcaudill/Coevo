...
In this experiment I am observing a co-evolving population (survival is determined by a random draw) with a non-interitable trait. I expect to see no spatial phenotype correlation 
...
inter_tall <- list.files(path="~/Desktop/no_interaction", pattern="_lit_", full.names=TRUE, recursive=FALSE)
inter_tall_cor_files <- list.files(path="~/Desktop/no_interaction", pattern="_cor_", full.names=TRUE, recursive=FALSE)
inter_inter_grid_tall_files <- list.files(path="~/Desktop/no_interaction", pattern="_grid_", full.names=TRUE, recursive=FALSE)


inter_tall_tall_file<- data.frame()
inter_cor_tall_file<- data.frame()
inter_inter_grid_tall_file<-data.frame()
name_list<-c()
cor_name_list<-c()
grid_name_list<-c()
for(i in seq(1, length(inter_tall), 1)){
  print(i)
  #sim
 
  
  inter_temp_file <- read.csv(inter_tall[i], header = TRUE)
  inter_temp_file$rep <- unlist(str_split(unlist(str_split(inter_tall[i],"rep_"))[2], "_"))[1]
  inter_temp_file$snake_mu_rate <- unlist(str_split(unlist(str_split(inter_tall[i],"snake_mu_rate_"))[2], "_"))[1]
  inter_temp_file$newt_mu_rate <- unlist(str_split(unlist(str_split(inter_tall[i],"newt_mu_rate_"))[2], "_"))[1]
  inter_temp_file$snake_mu_effect_sd <- unlist(str_split(unlist(str_split(inter_tall[i],"snake_mu_effect_sd_"))[2], "_"))[1]
  inter_temp_file$newt_mu_effect_sd <- unlist(str_split(unlist(str_split(inter_tall[i],"newt_mu_effect_sd_"))[2], "_"))[1]
  name_list<-c(paste0(inter_temp_file$snake_mu_rate[1], inter_temp_file$newt_mu_rate[1], inter_temp_file$snake_mu_effect_sd[1], inter_temp_file$newt_mu_effect_sd[1]),name_list)
  inter_temp_file$sim <- LETTERS[length(unique(name_list))]
  inter_tall_tall_file <- rbind(inter_tall_tall_file, inter_temp_file)
  print(inter_temp_file$sim[1])
  
  cor_temp_file <- read.csv(inter_tall_cor_files[i], header = TRUE)
  cor_temp_file$sim <-
  cor_temp_file$rep <-unlist(str_split(unlist(str_split(inter_tall_cor_files[i],"rep_"))[2], "_"))[1]
  cor_temp_file$snake_mu_rate <- unlist(str_split(unlist(str_split(inter_tall_cor_files[i],"snake_mu_rate_"))[2], "_"))[1]
  cor_temp_file$newt_mu_rate <- unlist(str_split(unlist(str_split(inter_tall_cor_files[i],"newt_mu_rate_"))[2], "_"))[1]
  cor_temp_file$snake_mu_effect_sd <- unlist(str_split(unlist(str_split(inter_tall_cor_files[i],"snake_mu_effect_sd_"))[2], "_"))[1]
  cor_temp_file$newt_mu_effect_sd <- unlist(str_split(unlist(str_split(inter_tall_cor_files[i],"newt_mu_effect_sd_"))[2], "_"))[1]
  ...
  inter_cor_tall_file <- rbind(inter_cor_tall_file, cor_temp_file)
  print(cor_temp_file$sim[1])
  
  inter_grid_temp_file <- read.csv(inter_inter_grid_tall_files[i], header = TRUE)
  inter_grid_temp_file$sim <-
  inter_grid_temp_file$rep <-unlist(str_split(unlist(str_split(inter_inter_grid_tall_files[i],"rep_"))[2], "_"))[1]
  inter_grid_temp_file$snake_mu_rate <- unlist(str_split(unlist(str_split(inter_inter_grid_tall_files[i],"snake_mu_rate_"))[2], "_"))[1]
  inter_grid_temp_file$newt_mu_rate <- unlist(str_split(unlist(str_split(inter_inter_grid_tall_files[i],"newt_mu_rate_"))[2], "_"))[1]
  inter_grid_temp_file$snake_mu_effect_sd <- unlist(str_split(unlist(str_split(inter_inter_grid_tall_files[i],"snake_mu_effect_sd_"))[2], "_"))[1]
  inter_grid_temp_file$newt_mu_effect_sd <- unlist(str_split(unlist(str_split(inter_inter_grid_tall_files[i],"newt_mu_effect_sd_"))[2], "_"))[1]
  grid_name_list<-c(paste0(inter_grid_temp_file$snake_mu_rate[1], inter_grid_temp_file$newt_mu_rate[1], inter_grid_temp_file$snake_mu_effect_sd[1], inter_grid_temp_file$newt_mu_effect_sd[1]),grid_name_list)
  inter_grid_temp_file$sim <- LETTERS[length(unique(grid_name_list))]
  inter_inter_grid_tall_file <- rbind(inter_inter_grid_tall_file, inter_grid_temp_file)
   print(inter_grid_temp_file$sim[1])
...
#cfile=inter_cor_tall_file 
  #pattern="1.0e-08_0.005_1.0e-09_0.0158_2"
...
file_name= "Snake_1.0e-11_5.0_Newt_1.0e-11_5.0_0"
...
inter_data_long_mean <- gatinter(inter_mean, condition, measurement, newt_mean_pheno_0:snake_mean_pheno_99, factor_key=TRUE)
