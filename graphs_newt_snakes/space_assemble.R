...
p2<-ggplot(space_data[which(space_data$gen == 1000),], aes(x=loc_x, y=loc_y, color=type)) +
...
p4<-ggplot(space_data[which(space_data$gen == 1000),], aes(x=loc_x, y=loc_y, color=type)) +
  ggtitle("End Space: Generation 1000", subtitle = "file_redo_su_1.11e-11_nu_1.11e-11_sue_0.3_nue_0.3_17018532.txt") +
  xlab("Longitude") + ylab("Latitude")+
...
p6<-ggplot(space_data[which(space_data$gen == 1000),], aes(x=ind_phy, color=type, fill=type), alpha=0.3) +
  ggtitle("End Space: Generation 1000", subtitle = text_sub_title) +
  xlab("Phenotype") + ylab("Count")+
  geom_histogram(position="identity", alpha=0.3)+
