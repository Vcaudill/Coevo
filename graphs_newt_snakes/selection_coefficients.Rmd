...
vc1 <- var_by_var(Newt_pop_size, Newt_mean_Pheno, Newt_mean_Pheno, Snake_pop_size, title="Newt VS Snake Population Size Color=Phenotype", subt="sigma = 0.1, mu = 1.0e-10", y_label="Snake Popsize", x_label="Newt Popsize", colored=TRUE, color_label="Newt Phenotype" , highcol="darkred") 

vc2 <- var_by_var(Newt_pop_size, Snake_mean_Pheno, Snake_mean_Pheno, Snake_pop_size, title="Newt VS Snake Population Size Color=Phenotype", subt="sigma = 0.1, mu = 1.0e-10", y_label="Snake Popsize", x_label="Newt Popsize", colored=TRUE, color_label="Snake Phenotype", highcol="steelblue") 
...
```{r, relationship of interaction and pop size}

population_size = 700
number_of_interaction = 40

interaction_rate = number_of_interaction/population_size

print(paste0("If the population size is ", population_size, " and the number of interactions is ", number_of_interaction, ", then the interation rate is ", interaction_rate, "."))
...
population_size = 700
number_of_interaction = population_size*interaction_rate
print(paste0("Now if I pick the interaction rate of ", interaction_rate, " and the number of individuals is ", population_size, ", the number of interactions should be ", number_of_interaction))


interaction_rate = 0.05
population_size = 3369
number_of_interaction = population_size*interaction_rate
print(paste0("Now if I pick the interaction rate of ", interaction_rate, " and the number of individuals is ", population_size, ", the number of interactions should be ", number_of_interaction))

var_by_var(newt_pop_size, newt_found, newt_found, snake_pop_size, title="Newt VS Snake Population Size Color = Number of Interaction", subt="sigma = 0.1, mu = 1.0e-10", y_label="Snake Population Size", x_label="Newt Population Size", colored=TRUE, color_label="Interaction") 
...
## How many newts are near a snake???

I had SLiM print out the amount of newts that were in the range of each newt for 1000 generations. The parameters/msprime file were  "newt_snake/data/both_1000_su_1e-10_nu_1e-10_sue_0.1_nue_0.1_try_this_peter.init.trees"

```{r, how many newts in range}
newts_in_snakes_range <- read.table("~/Desktop/how_many_newts_in_range.txt", header =FALSE)

hist(newts_in_snakes_range$V1, c="darkorchid4", main="Number of Newts Near a Snake", xlab="Number of Newts")
abline(v = mean(newts_in_snakes_range$V1), col = "black", lwd = 2)
```

This purple Loch Ness monster represents the number of newts that resign in a particulars snakes range. It seems the mean is around 4, so there are on average 4 newts in a snakes range. I got these values from this line of slim: i1.nearestNeighborsOfPoint(p1, mPoint, count=1000000), which might be something that I can control (change in the future?).   

## No Space SLiM 

Trying to make a realistic interaction between newts and snakes without space as a factor.
I am also trying to make reproduction more realistic. 

### Peters thoughts on the factors that will make the interaction more realstic 

Suppose that they all live in a rectangular valley measuring **W** x **L** meters square. Each snake moves around in a range of area **As**. Say there are **Ns** snakes and **Nn** newts.


1. How many newts on average are there in the range of a given snake? **As** * **Nn**/**W*****L**
2. What proportion of newts are within the range of a given snake? (**As** * **Nn**/**W*****L**)**Nn**
3. Suppose that when Nn == Ns we'd like each snake to encounter one newt on average every y years. What proportion of the newts within the snake's range should each snake encounter? ((**As** * **Nn**/**W*****L**)**Nn**)/**y**


```{r, first look at no space}
nospace_results <- read.table("~/Desktop/test_values_1on1_nospace.txt", header =TRUE)
